...
class Admin {
    ...
    public function getTopicWords($topicId) {
        $stmt = $this->db->prepare("
            ...
            FROM local_words lw
            INNER JOIN topic_words tw ON lw.id = tw.local_word_id
            WHERE tw.topic_id = ?
    ...
    public function assignWordsToTopic($topicId, $words) {
        ...
        try {
            foreach ($words as $word) {
                ...
                // Tìm từ vựng theo tên (case-insensitive)
                $stmt = $this->db->prepare("SELECT id FROM local_words WHERE LOWER(word) = LOWER(?) LIMIT 1");
                $stmt->execute([$word]);
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                ...
                if ($wordId > 0) {
                    // Thêm vào topic_words (đúng bảng)
                    $stmt = $this->db->prepare("
                        INSERT IGNORE INTO topic_words (topic_id, local_word_id) 
                        VALUES (?, ?)
                    ");
                    $stmt->execute([$topicId, $wordId]);
                }
            }

            return true;
        } catch (Exception $e) {
            error_log("Error in assignWordsToTopic: " . $e->getMessage());
            throw $e;
        }
    }

    public function removeAllWordsFromTopic($topicId) {
        $stmt = $this->db->prepare("
    ...
    public function removeWordFromTopic($wordId, $topicId) {
        $stmt = $this->db->prepare("
            DELETE FROM topic_words 
            WHERE local_word_id = ? AND topic_id = ?
        ");
