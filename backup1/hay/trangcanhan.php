...
<body>
    <?php define('INCLUDED', true); include 'includes/header.php'; ?>
    <div class="container animate-fade-in">
        <div class="other-reviews">
            <h3 style="text-align:center;">Các bài đánh giá của bạn</h3>
            <div id="reviews-container" class="reviews">
                <?php while ($review = mysqli_fetch_assoc($reviews_result)): ?>
                    <div id="review-<?php echo $review['review_id']; ?>" class="review-card card">
                        <div class="review-header">
                            <span class="author"><?php echo htmlspecialchars($user['username']); ?></span>
                            <span class="date"><?php echo date('d/m/Y, H:i', strtotime($review['created_at'])); ?></span>
                            <div class="review-actions">
                                <button type="button" class="btn btn-primary" onclick="showEditForm(<?php echo $review['review_id']; ?>)"><i class="fas fa-edit"></i> Chỉnh sửa</button>
                            </div>
                        </div>
                        <h3 class="store-name"><?php echo htmlspecialchars($review['store_name']); ?></h3>
                        <div class="rating-section">
                            <span class="store-address"><?php echo htmlspecialchars($review['address']); ?></span>
                        ...
                        <div id="edit-form-<?php echo $review['review_id']; ?>" class="edit-form" style="display:none;">
                            <form method="POST" action="update_review.php" enctype="multipart/form-data" class="edit-review-form">
                                ...
                                <div class="form-group">
                                    <label for="rating-<?php echo $review['review_id']; ?>">Đánh giá:</label>
                                    <select name="rating" id="rating-<?php echo $review['review_id']; ?>" required>
                                        <?php for($i = 1; $i <= 5; $i++): ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <button type="button" class="cancel-btn" onclick="hideEditForm(<?php echo $review['review_id']; ?>)">Hủy</button>
                                </div>
                            </form>
                        </div>
                        <?php
    ...
    <script src="assets/js/trangcanhan.js" defer></script>
</body>
</html>